...
{
    [TestFixture]
    class ReportersMenuCommand_Tests
    {
        [Test]
        public void ConstructorShould_ReturnInstanceOfReportersMenuCommandClass_WhenThePassedValuesAreValid()
        {
            var writerMock = new Mock<IWriter>();
            var command = new ReportersMenuCommand(writerMock.Object);

            Assert.IsInstanceOf<ReportersMenuCommand>(command);
        }

        [Test]
        public void ConstructorShould_ThrowArgumentNullException_WhenThePassedWriterIsNull()
        {
            Assert.Throws<ArgumentNullException>(() => new ReportersMenuCommand(null));
        }
